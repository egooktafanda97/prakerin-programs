    ...
    <div class="card mt-5">
        <div class="card-header mb-0 pl-5 pr-5 pt-5 pb-0">
            <div class="flex justify-between">
                <h4 class="card-title">Form Penempatan Prakerin</h4>
            </div>
        </div>
        <div class="card-body">
            @if (session('error'))
            ...
            @endif
            <form action="{{ route('penempatan.store') }}" class="px-5 pt-5 pb-2" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select class="mt-1 block w-full rounded border border-gray-300 p-2" id="siswa_id" name="siswa_id"
                            ...
                            @foreach ($siswa as $item)
                                <option {{ old('siswa_id') == $item->id ? 'selected' : '' }} value="{{ $item->id }}">
                                    {{ $item->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select class="mt-1 block w-full rounded border border-gray-300 p-2" id="perusahaan_id"
                            ...
                            @foreach ($perusahaan as $item)
                                <option {{ old('perusahaan_id') == $item->id ? 'selected' : '' }}
                                    value="{{ $item->id }}">
                    ...
                    <div>
                        ...
                        <select class="mt-1 block w-full rounded border border-gray-300 p-2" id="instruktur_id"
                            ...
                            @foreach ($instruktur as $item)
                                <option {{ old('instruktur_id') == $item->id ? 'selected' : '' }}
                                    value="{{ $item->id }}">
                                    {{ $item->nama }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input class="mt-1 block w-full rounded border border-gray-300 p-2" id="tanggal_mulai"
                            name="tanggal_mulai" required type="date" value="{{ old('tanggal_mulai') }}">
                    </div>
                    ...
                    <div>
                        ...
                        <input class="mt-1 block w-full rounded border border-gray-300 p-2" id="tanggal_selesai"
                            name="tanggal_selesai" required type="date" value="{{ old('tanggal_selesai') }}">
                    </div>
