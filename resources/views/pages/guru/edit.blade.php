    ...
    <div class="card mt-5">
        <div class="card-header mb-0 pl-5 pr-5 pt-5 pb-0">
            <div class="flex justify-between">
                <h4 class="card-title">Update Data Guru & Akun</h4>
            </div>
        </div>
        <div class="card-body">
            @if (session('error'))
            ...
            @endif
            <form action="{{ route('guru.update', $item->id) }}" class="px-5 pt-5 pb-2" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                            id="alamat" name="alamat" placeholder="Masukkan alamat lengkap guru" rows="3">{{ old('alamat', $item->alamat) }}</textarea>
                    </div>
                </div>
